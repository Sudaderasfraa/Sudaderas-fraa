...
const productosData = {
    ...
    p1: {
        ...
        precio: 612,
        descripcion: '2 sudaderas con colores y tallas a elección. La segunda sudadera con 25% de descuento. Una para casa, una para salir.',
        cantidadPrendas: 2,
        colores: {
            azul: {
                nombre: 'Azul',
                imagenPrincipal: 'img/products/f1.jpeg',
                imagenesSecundarias: ['img/products/f1.jpeg', 'img/products/f3.jpeg', 'img/products/f2.jpeg', 'img/products/f4.jpeg']
            },
            gris: {
                nombre: 'Gris',
                imagenPrincipal: 'img/products/f3.jpeg',
                imagenesSecundarias: ['img/products/f3.jpeg', 'img/products/f2.jpeg', 'img/products/f4.jpeg', 'img/products/f1.jpeg']
            },
            beige: {
                nombre: 'Beige',
                imagenPrincipal: 'img/products/f2.jpeg',
                imagenesSecundarias: ['img/products/f2.jpeg', 'img/products/f4.jpeg', 'img/products/f1.jpeg', 'img/products/f3.jpeg']
            },
            verde: {
                nombre: 'Verde',
                imagenPrincipal: 'img/products/f4.jpeg',
                imagenesSecundarias: ['img/products/f4.jpeg', 'img/products/f1.jpeg', 'img/products/f2.jpeg', 'img/products/f3.jpeg']
            }
        },
        tallas: ['Chica', 'Mediana', 'Grande']
    },
    p2: {
        ...
        nombre: 'Pack Tonal',
        precio: 840,
        descripcion: '3 sudaderas con tonos neutros coordinados. La tercera sudadera con 20% de descuento. Tu guardarropa de básicos completo en una sola compra.',
        cantidadPrendas: 3,
        colores: {
            beige: {
                nombre: 'Beige',
                imagenPrincipal: 'img/products/f2.jpeg',
                imagenesSecundarias: ['img/products/f2.jpeg', 'img/products/f3.jpeg', 'img/products/f4.jpeg', 'img/products/f1.jpeg']
            },
            gris: {
                nombre: 'Gris',
                imagenPrincipal: 'img/products/f3.jpeg',
                imagenesSecundarias: ['img/products/f3.jpeg', 'img/products/f4.jpeg', 'img/products/f1.jpeg', 'img/products/f2.jpeg']
            },
            verde: {
                nombre: 'Verde',
                imagenPrincipal: 'img/products/f4.jpeg',
    ...
    p3: {
        id: 'p3',
        nombre: 'Pack Duo Regalo',
        precio: 665,
        descripcion: '2 sudaderas más una nota personalizable. El regalo que no falla. Listo para entregar.',
        cantidadPrendas: 2,
        colores: {
            azul: {
                nombre: 'Azul',
    ...
    p4: {
        id: 'p4',
        nombre: 'Pack Familiar/Pareja',
        precio: 612,
        descripcion: '2 tallas distintas para pareja o familia. Para los dos.',
        cantidadPrendas: 2,
        colores: {
            azul: {
                nombre: 'Azul',
    ...
    p5: {
        id: 'p5',
        nombre: 'Pack Coleccionista 5x',
        precio: 1400,
        descripcion: '5 sudaderas, una de cada color. Toda la paleta. Una inversión, multiples looks.',
        cantidadPrendas: 5,
        colores: {
            azul: {
                nombre: 'Azul',
...
function agregarAlCarrito(producto) {
    ...
    const { id, nombre, precio, imagen, talla, color, cantidad, esPack = false } = producto;
    ...
    if (existente) {
        ...
        carrito.push({ id, nombre, precio, imagen, talla, color, cantidad, esPack });
    }
...
function obtenerPrefijoPack(id) {
    return id.includes('-') ? id.split('-')[0] : id;
}

function eliminarDelCarrito(id, talla, color) {
    let carrito = obtenerCarrito();
    const item = carrito.find(item => item.id === id && item.talla === talla && item.color === color);
    if (!item) return;

    if (item.esPack) {
        const prefijoPack = obtenerPrefijoPack(id);
        carrito = carrito.filter(item => !item.id.startsWith(prefijoPack + '-'));
        guardarCarrito(carrito);
        renderizarCarrito();
        mostrarNotificacion('🗑️ Paquete completo eliminado');
        return;
    }
...
function renderizarCarrito() {
    ...
    carrito.forEach(item => {
        ...
        const eliminarCelda = `<a href="#" class="eliminar-item" data-id="${item.id}" data-talla="${item.talla}" data-color="${item.color}" title="Quitar paquete completo"><i class="far fa-times-circle"></i></a>`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Anuncio flotante en el centro con opción de cerrar
    ...
    if (heroAnnouncement) {
        ...
        const applyAnnouncementPosition = () => {
            heroAnnouncement.style.transform = `translate(calc(-50% + ${currentX}px), calc(-50% + ${currentY}px))`;
        };
        ...
        const onPointerMove = (event) => {
            ...
            currentX = moveX - dragStartX;
            currentY = moveY - dragStartY;
            applyAnnouncementPosition();
        };

        const stopDragging = () => {
            if (!isDragging) return;
            isDragging = false;
            heroAnnouncement.classList.remove('dragging');
        ...
        heroAnnouncement.addEventListener('pointerdown', (event) => {
            if (event.target.classList.contains('announcement-close')) return;
            isDragging = true;
            dragStartX = event.clientX - currentX;
            dragStartY = event.clientY - currentY;
            heroAnnouncement.classList.add('dragging');
            window.addEventListener('pointermove', onPointerMove);
            window.addEventListener('pointerup', stopDragging);
            event.preventDefault();
        });

        heroAnnouncement.addEventListener('touchstart', (event) => {
            if (event.target.classList.contains('announcement-close')) return;
            isDragging = true;
            dragStartX = event.touches[0].clientX - currentX;
            dragStartY = event.touches[0].clientY - currentY;
            heroAnnouncement.classList.add('dragging');
        ...
        // Botón cerrar
        const closeBtn = heroAnnouncement.querySelector('.announcement-close');
        if (closeBtn) {
    ...
    document.querySelectorAll('.pack-card').forEach(card => {
        ...
        const btnDetalles = card.querySelector('.pack-btn-detalles');
        const btnCarrito = card.querySelector('.pack-btn-carrito');
        const btnCerrar = card.querySelector('.pack-btn-cerrar');
        const packOptions = card.querySelector('.pack-options');
        const packPrendas = card.querySelector('.pack-prendas');
        const packDetails = card.querySelector('.pack-details');

        let btnX = card.querySelector('.pack-btn-x');
        if (!btnX && packDetails && btnDetalles) {
            btnX = document.createElement('button');
            btnX.type = 'button';
            btnX.className = 'pack-btn-x';
            btnX.title = 'Abrir detalles del paquete';
            btnX.textContent = '×';
            btnDetalles.insertAdjacentElement('afterend', btnX);
        }

        // Función para cerrar opciones
        const cerrarOpciones = () => {
            packOptions.style.display = 'none';
            btnDetalles.style.display = 'block';
        };

        // Click en botón cerrar
        if (btnCerrar) {
            btnCerrar.addEventListener('click', cerrarOpciones);
        }

        // Click en la x para abrir detalles
        if (btnX) {
        ...
        if (btnDetalles) {
            btnDetalles.addEventListener('click', (e) => {
                e.preventDefault();
                const pack = productosData[packId];
                if (!pack) return;

                // Generar selectores de color y talla para cada prenda
                packPrendas.innerHTML = '';
                for (let i = 1; i <= pack.cantidadPrendas; i++) {
                    const prendasel = document.createElement('div');
                    prendasel.className = 'prenda-selector';
                    prendasel.dataset.prenda = i;

                    // Selector de color
                    const labelColor = document.createElement('label');
                    labelColor.textContent = `Sudadera ${i}:`;
                    const selectColor = document.createElement('select');
                    selectColor.className = 'color-select';
                    selectColor.innerHTML = '<option value="">Color</option>';
                    Object.keys(pack.colores).forEach(colorKey => {
                        const option = document.createElement('option');
                        option.value = colorKey;
                        option.textContent = pack.colores[colorKey].nombre;
                        selectColor.appendChild(option);
                    });

                    // Selector de talla
                    const selectTalla = document.createElement('select');
                    selectTalla.className = 'size-select';
                    selectTalla.innerHTML = '<option value="">Talla</option>';
                    pack.tallas.forEach(talla => {
                        const option = document.createElement('option');
                        option.value = talla;
                        option.textContent = talla;
                    ...
                    prendasel.appendChild(labelColor);
                    prendasel.appendChild(selectColor);
                    prendasel.appendChild(selectTalla);
                    packPrendas.appendChild(prendasel);
                }

                // Mostrar opciones
                packOptions.style.display = 'block';
                btnDetalles.style.display = 'none';
            });
        }

        // Click en "Agregar al carrito" del paquete
        if (btnCarrito) {
            btnCarrito.addEventListener('click', () => {
                const pack = productosData[packId];
                if (!pack) return;

                const prendas = card.querySelectorAll('.prenda-selector');
                let todasValidas = true;

                prendas.forEach((prenda, index) => {
                    const color = prenda.querySelector('.color-select').value;
                    const talla = prenda.querySelector('.size-select').value;
                ...
                if (!todasValidas) {
                    mostrarNotificacion('⚠️ Selecciona color y talla para todas las sudaderas');
                    return;
                }

                // Agregar cada prenda al carrito
                prendas.forEach((prenda, index) => {
                    const color = prenda.querySelector('.color-select').value;
                    const talla = prenda.querySelector('.size-select').value;
                    const colorData = pack.colores[color];

                    agregarAlCarrito({
                        id: packId + '-' + (index + 1),
                        nombre: pack.nombre + ' - Sudadera ' + (index + 1),
                        precio: pack.precio / pack.cantidadPrendas,
                        imagen: colorData.imagenPrincipal.split('/').pop(),
                        talla: talla,
    ...
    // ========================
    // Botón flotante Ver detalles
    // ========================
    const floatingBtn = document.querySelector('.floating-details-btn');
    const quickSelectModal = document.getElementById('quick-select-modal');
    const quickSelectPrendas = document.getElementById('quick-select-prendas');
    const quickSelectCloseBtn = document.querySelector('.quick-select-close');
    const quickSelectAddBtn = document.getElementById('quick-select-add-btn');
    const quickSelectOverlay = document.querySelector('.quick-select-overlay');
    ...
    if (floatingBtn) {
        ...
        // Hacer que se mueva al scrollear (efecto parallax suave)
    ...
    if (floatingBtn) {
        floatingBtn.addEventListener('click', (e) => {
            if (isDragging) return; // No abrir modal si estamos arrastrando
            const pack = productosData['p2']; // Pack Tonal con 3 sudaderas
            if (!pack) return;

            // Generar selectores
            quickSelectPrendas.innerHTML = '';
            for (let i = 1; i <= 3; i++) {
                const prendasel = document.createElement('div');
                prendasel.className = 'quick-prenda-selector';
                prendasel.dataset.prenda = i;

                const titulo = document.createElement('h4');
                titulo.textContent = `Sudadera ${i}`;
                prendasel.appendChild(titulo);

                const fila = document.createElement('div');
                fila.className = 'quick-selector-row';
                ...
                fila.appendChild(labelColor);
                fila.appendChild(selectColor);
                prendasel.appendChild(fila);

                // Selector de talla
                const fila2 = document.createElement('div');
                fila2.className = 'quick-selector-row';
                ...
                fila2.appendChild(labelTalla);
                fila2.appendChild(selectTalla);
                prendasel.appendChild(fila2);

                quickSelectPrendas.appendChild(prendasel);
            }
    ...
    // Cerrar modal
    if (quickSelectCloseBtn) {
        quickSelectCloseBtn.addEventListener('click', () => {
            quickSelectModal.style.display = 'none';
        });
    }

    if (quickSelectOverlay) {
        quickSelectOverlay.addEventListener('click', () => {
            quickSelectModal.style.display = 'none';
        });
    }

    // Agregar al carrito
    if (quickSelectAddBtn) {
        quickSelectAddBtn.addEventListener('click', () => {
            const pack = productosData['p2'];
            if (!pack) return;

            const prendas = document.querySelectorAll('.quick-prenda-selector');
            let todasValidas = true;
